...
int user_register( char *reg_buf )
{
    ...
    char tel[128];
    char email[128];
    ret = get_reg_info(reg_buf, user_name, nick_name, pwd, tel, email);// 解析json串
    if(ret != 0)
    {
        goto END;
    }
    ...
    conn = msql_conn(mysql_user, mysql_pwd, mysql_db); // 连接数据库
    if (conn == NULL)
    {
...
int main()
{
    ...
    while (FCGI_Accept() >= 0) 
    {
        char *contentLength = getenv("CONTENT_LENGTH"); // 内容长度 http body的数据
        int len;
        ...
        else //获取登陆用户信息
        {
            ...
            int ret = 0;
            char *out = NULL;
            ret = fread(buf, 1, len, stdin); //从标准输入(web服务器)读取内容 http body内容读取出来
            if(ret == 0)
            {
                LOG(REG_LOG_MODULE, REG_LOG_PROC, "fread(buf, 1, len, stdin) err\n");
                continue;
            }
// buf = {"email":"","firstPwd":"9eaacf710df4ed48ccfd2fec40da479b","nickName":"江水啊江水","phone":"","userName":"zhangwei"}
